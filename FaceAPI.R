...
q <- "?returnFaceId=true&returnFaceLandmarks=true&returnFaceAttributes=age,gender,smile,headPosefacialHair,glasses,emotion"
url <- paste(baseUrl, q, sep="")

img1 <- "http://vis-www.cs.umass.edu/lfw/images/Jack_Nicholson/Jack_Nicholson_0002.jpg"
img2 <- "http://vis-www.cs.umass.edu/lfw/images/Jack_Nicholson/Jack_Nicholson_0003.jpg"
...
response = POST(url=url1, body=pic1, add_headers(.headers = 
          c('Content-Type'='application/octet-stream', 'Ocp-Apim-Subscription-Key'='xxxxxxxxxxxxxxxxxxxxxxxxxxxxx')))
...
points(tip_nose_x, tip_nose_y, pch=19, col="red")

#now let's draw the dot at that location
points(tip_nose_x, IMAGE_HEIGHT - tip_nose_y, pch=19, col="red")
# left pupil location
left_pupil_x = df[ ,c("faceLandmarks.pupilLeft.x")]
left_pupil_y = df[ ,c("faceLandmarks.pupilLeft.y")]
points(left_pupil_x, IMAGE_HEIGHT - left_pupil_y, pch=19, col="red")
...
responseSimilar = POST(url=baseUrlFindSimilar, body=bodyFindSimilar, add_headers(.headers =
                                                                                   ...
                                                                                   c('Content-Type'='application/json', 'Ocp-Apim-Subscription-Key'='xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')))
